...
export default function WhyChooseUs() {
  ...
  const desktopMainCardRef = useRef(null);
  const desktopTitleSectionRef = useRef(null);
  const goalCardRef = useRef(null);
  const resultCardRef = useRef(null);
  const mobileTitleSectionRef = useRef(null);
  const mobileGoalCardRef = useRef(null);
  const mobileResultCardRef = useRef(null);
  const mobileMainCardRef = useRef(null);

  useEffect(() => {
    const isRtl = t('direction') === 'rtl';
    const animationClass = isRtl ? styles.cardAlt : styles.card; // slideInFromLeft for RTL, slideInFromRight for LTR
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        // Collect all parent elements for the current view
        const elements = entry.target === desktopContainerRef.current
          ? [
              desktopMainCardRef.current,
              desktopTitleSectionRef.current,
              goalCardRef.current,
              resultCardRef.current,
            ]
          : [
              mobileTitleSectionRef.current,
              mobileGoalCardRef.current,
              mobileResultCardRef.current,
              mobileMainCardRef.current,
            ].filter(Boolean);

        if (entry.isIntersecting) {
          // Apply animation to all parent elements simultaneously
          elements.forEach((el) => {
            if (el) {
              el.classList.add(animationClass);
              el.classList.remove(styles.noAnimation);
            }
          });
        } else {
          // Reset to allow replay when out of view
          elements.forEach((el) => {
            if (el) {
              el.classList.remove(styles.card, styles.cardAlt);
    ...
    // Observe the desktop and mobile containers
    const containers = [desktopContainerRef.current, mobileContainerRef.current].filter(Boolean);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] py-16 px-4 relative overflow-hidden" dir={t('direction')}>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div ref={desktopContainerRef} className="hidden lg:grid lg:grid-cols-2 gap-12 items-start text-left">
          <div className="relative order-first lg:order-first">
            <div
              ref={desktopMainCardRef}
              style={{
                backgroundImage: `url(${textbg})`,
                backgroundSize: 'cover',
                backgroundPosition: 'center',
              }}
              className={`${styles.noAnimation} rounded-3xl p-12 border border-slate-700/50 shadow-2xl relative overflow-hidden bg-slate-900/40 backdrop-blur-sm`}
            >
          ...
          <div className="space-y-8 text-left order-last lg:order-last">
            <div ref={desktopTitleSectionRef} className={`${styles.noAnimation}`}>
              ...
              <h1 className="text-5xl font-bold text-white mb-6 leading-tight text-left">
                {t('designsThatDriveSales').split(' ').slice(0, 2).join(' ')}{' '}
                <span className="text-blue-500">{t('designsThatDriveSales').split(' ').slice(2).join(' ')}</span>
              </h1>
              <p className="text-slate-400 text-base leading-relaxed mb-10 text-left">
            ...
            <div className="space-y-6 text-left">
              <div
                ...
                className={`${styles.noAnimation} rounded-2xl p-6 border border-slate-700/50 hover:border-blue-500/50 transition-all duration-300 text-left bg-transparent backdrop-blur-sm`}
              >
        ...
        <div ref={mobileContainerRef} className="lg:hidden space-y-8 text-left">
          <div ref={mobileTitleSectionRef} className={`${styles.noAnimation}`}>
            <div className="inline-block px-4 py-2 bg-slate-800/70 rounded-full text-xs text-slate-300 mb-6 border border-slate-700 text-left">
              {t('whyChooseUs')}
            </div>
            ...
            <div className="space-y-6 px-4 text-left">
              <div
                ref={mobileGoalCardRef}
                className={`${styles.noAnimation} rounded-2xl p-6 border border-slate-700/50 bg-transparent backdrop-blur-sm`}
              >
          ...
          <div className="px-4 text-left">
            <div
              ref={mobileMainCardRef}
              style={{
                backgroundImage: `url(${textbg})`,
                backgroundSize: 'cover',
                backgroundPosition: 'center',
              }}
              className={`${styles.noAnimation} rounded-3xl p-6 border border-slate-700/50 bg-transparent backdrop-blur-sm`}
            >
              <div className="relative z-10">
                <div className="inline-block px-4 py-2 bg-slate-900/70 rounded-full text-xs text-slate-300 mb-8 border border-slate-700 text-left">
                  {t('whatSetsUsApart')}
